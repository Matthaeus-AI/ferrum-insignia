...
// Am Ende von home.js hinzufügen
document.addEventListener('DOMContentLoaded', function() {
  // Responsivitäts-Fallback-Trigger
  const applyResponsiveFallbacks = function() {
    if (window.innerWidth <= 991) {
      // DOM-Selektoren für kritische Elemente
      ...
      // Sicherstellen, dass Produktkarten sichtbar sind
      document.querySelectorAll('.product-card').forEach(card => {
        card.style.opacity = '1';
        card.style.visibility = 'visible';
        card.setAttribute('data-manual-visibility', 'true');
      });
    }
  };
  
  // Initial auslösen und bei Größenänderung
  applyResponsiveFallbacks();
  window.addEventListener('resize', applyResponsiveFallbacks);
});
});