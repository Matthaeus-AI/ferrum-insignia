...
class HistoryController {
    ...
    constructor() {
        ...
        // Geschwindigkeitsfaktor (leicht erhöht von 0.12 auf 0.15)
    ...
    bindMethods() {
        ...
        this.handleHeaderTemplateLoad = this.handleHeaderTemplateLoad.bind(this);
    }
     ...
     */
    initialize() {
        // Event-Listener für Template-Ladung
        document.addEventListener('template:loaded', this.handleHeaderTemplateLoad);
    ...
    /**
     * Handler für Template-Ladung-Event
     * @private
     * @param {CustomEvent} event - Event-Objekt
     * @return {void}
     */
    handleHeaderTemplateLoad(event) {
        // Wenn der spezielle History-Header geladen wurde
        if (event.detail && event.detail.id === 'header-container') {
            // UI-Elemente neu referenzieren nach DOM-Änderungen
            this.domRefs = new Proxy({}, { get: this.domRefs.__proto__.__get });
            
            // Inhaltsbeobachtung neu initialisieren
            this.observeContent();
            
            console.log('[HistoryController] Template geladen, Referenzen aktualisiert');
        }
    }
    ...
    createParticles() {
        ...
        for (let i = 0; i < actualCount; i++) {
            this.particles.push({
                ...
                velocity: {
                    // Geschwindigkeit leicht erhöht (von 0.12 auf 0.15)
                    x: (Math.random() - 0.5) * this.velocityFactor,
                    y: (Math.random() - 0.5) * this.velocityFactor
                },
    ...
    updateParticles() {
        ...
        this.particles.forEach(p => {
            ...
            // Position mit optimierter Bewegungsgeschwindigkeit (Dämpfungsfaktor entfernt)
            p.x += p.velocity.x;
            p.y += p.velocity.y;
     ...
     */
    observeContent() {
        // Alte Observer trennen, falls vorhanden
    ...
    /**
     * Implementiert viewport-spezifische Anpassungen für bessere Darstellung
     * @private
     * @return {void}
     */
    ...
    /**
     * Passt Darstellung basierend auf Viewport-Größe an
     * @private
     * @return {void}
     */
    handleViewportSizeChange() {
        ...
        // Viewport-Höhen-Anpassungen für optimale Darstellung
        document.documentElement.style.setProperty('--vh', `${vh * 0.01}px`);
        
        // Dynamische Partikelgeschwindigkeit basierend auf Viewport-Größe
        // Für kleinere Screens leicht reduzierte Geschwindigkeit für optimale Darstellung
        if (vw < 768) {
            this.velocityFactor = 0.125; // Leicht reduziert für mobile Ansicht
        } else {
            this.velocityFactor = 0.15; // Standard für Desktop
        }
        
        // Partikel mit angepasster Geschwindigkeit neu erstellen
        if (this.particles.length > 0) {
    ...
    destroy() {
        ...
        document.removeEventListener('template:loaded', this.handleHeaderTemplateLoad);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('template:loaded', (event) => {
        ...
        if (window.historyController && event.detail && event.detail.id === 'header-container') {
